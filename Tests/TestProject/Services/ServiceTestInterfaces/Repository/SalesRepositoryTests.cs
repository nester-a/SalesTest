...
namespace TestProject.Services.ServiceTestInterfaces.Repository
{
    public class SalesRepositoryTests : BaseTest
    {
        ...
        [Fact]
        public void SalesRepository_Delete_Test()
        {
            var list = new List<ISalesData>()
            {
                new SalesData()
            ...
            var deleteResult = repo.Delete(result1);
            repo.Save();

            Assert.True(deleteResult is not null);
            Assert.True(deleteResult.Id == 1);
            Assert.True(deleteResult.SalesPointId == sales1.SalesPointId);
            Assert.True(deleteResult.Date == sales1.Date);
            Assert.True(deleteResult.Time == sales1.Time);
            Assert.True(deleteResult.BuyerId is null);
            Assert.True(deleteResult.SalesData.Count == 2);

            Assert.True(db.Sales.Count() == 1);
        }
    }
}
